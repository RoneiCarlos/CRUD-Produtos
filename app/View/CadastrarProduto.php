...
<form class="registrar" enctype="multipart/form-data" action="<?= $editar? '/atualizar-produto?id='.$id : '/salvar-produto' ?>" method="POST">
    ...
    <div class="inp">
        ...
        <input id="nome" name="nome" type="text" placeholder="Nome do produto" <?= $editar ? 'value="'.$nome.'"' : ''?> required/>
    </div>
    <div class="inp">
        <label for="descricao">Descrição do produto</label>
        <textarea id="descricao" name="descricao" cols="40" rows="5" maxlength="500" wrap="hard" placeholder="Descrição do produto" required><?= $editar ? $descricao : '' ?></textarea>
    </div>
    <div class="inp">
